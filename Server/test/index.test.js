...
describe('Test de RUTAS', ()=>{
    describe('GET /rickandmorty/character/:id',()=>{
        ...
        it('Si hay un error responde con status: 500',async ()=>{
            await agent.get('/rickandmorty/character/8996').expect(500);
        })
    })

    describe('GET /rickandmorty/login', ()=>{
        it('Ruta de login retorna access true', async ()=>{
            const response = (await agent.get('/rickandmorty/login?email=negro@henry.com&password=Negro123')).body;
            expect(response.access).toEqual(true)
        })
        it('Ruta de login retorna access false', async ()=>{
            const response = (await agent.get('/rickandmorty/login?email=negro45&password=45663as')).body;
    ...
    describe('DELETE /rickandmorty/fav/:id', ()=>{
        ...
        it('Devuelve los personajes existentes luego de ID invalido', async ()=>{
            const response = (await agent.delete('/rickandmorty/fav/9566')).body;
            expect(response).toContainEqual(character1);
            expect(response).toContainEqual(character2);
        })
